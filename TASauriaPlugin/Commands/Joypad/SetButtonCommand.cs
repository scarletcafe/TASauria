...
public class SetButtonCommand : EmulatorCommand<SetButtonInput, SetButtonOutput>
{
    ...
    public override SetButtonOutput RunSync(EmulatorInterface emulator, Dictionary<string, string> arguments, SetButtonInput payload)
    {
        ...
        if (dictionary.TryGetValue(payload.Name, out object old)) {
            if (old is bool val) {
                value = val;
                emulator.APIs.Joypad.Set(payload.Name, payload.Value, payload.Controller);
            } else {
